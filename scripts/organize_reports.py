...
Handles multiple naming conventions for TXT files.
"""
import shutil
...
def main():
    for quarter, cfg in QUARTERS.items():
        ...
        for folder in sorted(src_dir.iterdir()):
            ...
            # Accept both digit-only (1101) and digit+name (1101台泥) folder names
            # Strip Chinese chars to get pure stock ID for non-digit folders
            import re
            ...
            # Copy TXT - collect from all matching globs
            txt_files = []
            for g in cfg.get("txt_globs", []):
                txt_files.extend(folder.glob(g))
            txt_files = list(set(txt_files))  # dedupe
            if txt_files:
                txt_src = txt_files[0]
            ...
            # Copy MD
            md_files = list(folder.glob(cfg["md_glob"]))
            if md_files:
                md_src = md_files[0]
